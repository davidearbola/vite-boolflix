...
export default {
	...
	created() {
		this.MyFunction.getHomeMovie(Store.upComing, "upComingMovie");
	},
};
</script>

<template>
	<h4 v-if="!Store.inputValue">Coming Soon</h4>
	<div
	...
	>
		<template v-for="(film, i) in Store.upComingMovie">
			<div
				...
				@click="visibleInfo(i, Store.upComingMovie[i].id)"
			>
				<SingleCard :elemento="film" />
			</div>
			<InfoCard
				v-if="visible"
				:elemento="Store.upComingMovie[active]"
				:cast="Store.credits"
			/>
			<i
				v-if="visible"
